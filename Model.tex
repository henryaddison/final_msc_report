\chapter{Model}
  ...
  \section{Overview}
    ...
    \section{Boat}
      ...
      \subsection{Movement}
      Unless directed to stop by the cox, a boat will continue to move forward. In part this is due to physics, even a eight a small women weighs over half a tonne. However, this is mainly due to the way rowers behave. Rowers are taught to carry on rowing unless the cox tells them to stop rather than being told to take each stroke individually. Therefore movement is a feature of the boat rather than the cox.
        
      For the purposes of this model, it is assumed that steering to avoid the bank happens automatically. The river is unchanging and there is no advantage to hitting the river bank. Although steering is in the cox's domain, for simplicity it has been absorbed into forward movement. An experienced cox will find that steering the river course becomes automatic anyway and it is only when avoiding moving obstacles like other boats that a cox will knowingly think about steering. More on this part of steering can be found in the Changing Lane action in \ref{model:cox:actions}.
                
      \subsection{Crashing}\label{model:movement:crashing}
        To properly model movement, it is also necessary to model when movement is not possible. That is when a boat crashes. Since steering to avoid the bank happens automatically, the crashes of interest are between multiple boats (mostly two but it does not take much for others to get involved). For this project crashes are considered to cause delays when two or more boats get two close to each other. In theory a crash could be so bad that the boats are written off and the crew must get out and walk back. This is exceedingly rare. Therefore it is assumed that crashes are not catastrophic. Instead when boats get too close someone either in the boat or on the bank will shout at them all to stop. The crews will then take some time to put some space between each other. This may involve pushing each other away physically or one boat drawing its oars in so that another may pass, or a boat may chose to "back it down", where the rowers will row backwards causing the boat to move slowly backwards.
